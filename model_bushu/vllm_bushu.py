...
# 创建了一个 FastAPI 应用实例，并指定了生命周期管理器
app = fastapi.FastAPI(lifespan=lifespan)


# 定义了一个自定义的 argparse.Action 类 LoRAParserAction，用于解析 LoRA 模块配置
...
# 定义了一个函数 parse_args，用于解析命令行参数，包括主机、端口、日志级别等
def parse_args():
...
if __name__ == "__main__":
    args = parse_args()
    args.model = "/DATA/LLM_model/deepseek/DeepSeek-R1-Distill-Qwen-1.5B"
    args.served_model_name = "DeepSeek-R1-Distill-Qwen-1.5B"
    args.gpu_memory_utilization = 0.9
    args.tensor_parallel_size = 2
    args.api_key = "sk-1234567890"

    args.max_model_len = 20000
    args.stream = False
    app.add_middleware(
        CORSMiddleware,
